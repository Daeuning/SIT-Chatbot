  ...
  try {
    ...
    const response = await openai.chat.completions.create({
      ...
      messages: [
        ...
        { role: "system", content: `3. 다음 조건을 만족할 경우 2를 반환해.
          1) 사용자가 p, q 임의 선택을 요청하고 gpt가 두 소수 p, q를 제공한 뒤 그 곱을 제공했을 경우
          2) 사용자가 소수의 곱을 요청하고 gpt가 그 값을 계산한 답변이 적절하다고 판단될 경우
          3) 사용자가 N값을 요청하고 gpt가 두 소수의 곱을 계산한 답변을 제공하였으며, 적절하다고 판단될 경우
          조건을 만족하지 않을 경우에는 false라는 단어만 반환해`},
        { role: "system", content: `4. 다음 조건을 만족할 경우 3을 반환해.
          1) 오일러 토션트 함수과 관련된 계산을 요청하고 그에 대한 gpt의 답변이 적절하다 판단될 경우
          2) 사용자가 φ(N) 계산을 요청하고, 그에 대한 gpt의 답변이 적절하다 판단될 경우
          3) 사용자가 오일러 함수와 관련된 계산을 요청하고, 그에 대한 gpt의 답변이 적절하다 판단될 경우
          3) gpt가 φ(N) 계산 결과를 제공했을 경우
          조건을 만족하지 않을 경우에는 false라는 단어만 반환해
        `},
        ...
        { role: "system", content: `7. 다음 조건을 만족할 경우 6을 반환해.
          1) GPT의 답변에 실제로 문자를 암호화한 숫자로된 결과가 포함되어 있어야 해.
          2) 사용자 질문에 공개 키와 개인 키에 관한 내용이 포함되어 있어야해
          3) 사용자 질문에 어떤 문자를 주고 RSA를 통해 암호화를 해달라는 내용이 포함되어야 있어야해
          조건을 만족하지 않을 경우에는 false라는 단어만 반환해`},
